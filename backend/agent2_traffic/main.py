...
<<<<<<< HEAD
    ...
    import hashlib
    fingerprint = hashlib.md5(f"{client_ip}:{path}:{method}:{payload}:{event_type}:{timestamp_sec}".encode()).hexdigest()
...
<<<<<<< HEAD
                    if assessment.score > 90:
                        ...
                        if app_config.protection_enabled:
                            ...
                            # Publish the high-risk attack event to Redis for stats and dashboard mapping
                            attack_event = {
                                "client_ip": client_ip,
                                "timestamp": datetime.utcnow().isoformat() + "Z",
                                "path": meta.path,
                                ...
                                "method": meta.method,
                                "payload": meta.payload[:200] if meta.payload else "",
                                "host": meta.host,
                                "action": "shadow_redirect",
                                "msg_id": request_id
                            }
                            ...
                            if 'high_frequency' in assessment.tags:
                                attack_event["action"] = "block"
                                await redis_client.publish("risk.events", attack_event)
                                await redis_client.redis.sadd("security.blocklist", client_ip)
